...
#pragma warning restore CA1062 // Validate arguments of public methods
        ...
        public static Exception GetExceptionForInvalidType<T>(in object obj, in string argumentName) => new ArgumentException($"{argumentName} must be an instance of {typeof(T).ToString()}. {argumentName} is an instance of {obj.GetType().ToString()}", argumentName);

        /// <summary>
        /// If <paramref name="obj"/> is not <typeparamref name="T"/>, throws the exception that is returned by the <see cref="GetExceptionForInvalidType{T}(in object, in string)"/> method.
        /// </summary>
        /// <typeparam name="T">The type to check.</typeparam>
        /// <param name="obj">The object to check.</param>
...

        {
            ...
            if (!(obj is T))
                ...
                throw GetExceptionForInvalidType<T>(obj, argumentName);

        }

        public static Exception GetExceptionForNotTypeOrNull<T>(in object obj, in string argumentName) where T : class => obj switch
        {
            null => GetArgumentNullException(argumentName),
            _ => (obj as T) switch
            {
                null => GetExceptionForInvalidType<T>(obj, argumentName),
                _ => null,
            },
        };
        ...
        /// <param name="obj">The object to check.</param>
        /// <param name="argumentName">The argument name for the <see cref="Exception"/> that is thrown.</param>
        public static void ThrowIfNotTypeOrNull<T>(in object obj, in string argumentName) where T : class => throw GetExceptionForNotTypeOrNull<T>(obj, argumentName);

        public static T GetOrThrowIfNotType<T>(in object obj, in string argumentName) where T : struct => obj is T _obj ? _obj : throw GetExceptionForInvalidType<T>(obj, argumentName);

        public static T GetOrThrowIfNotTypeOrNull<T>(in object obj, in string argumentName) where T : class => (obj ?? throw GetArgumentNullException(argumentName)) is T _obj ? _obj : throw GetExceptionForInvalidType<T>(obj, argumentName);

        public static InvalidOperationException GetExceptionForDispose(in string objectName, in bool forWhenDisposing)
        {
            if (forWhenDisposing)

                return new ObjectDisposingException(objectName, "The current object or value is disposing.");
