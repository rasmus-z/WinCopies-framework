namespace SevenZip
{
...
#endif
    {
        ...
        public SevenZipExtractor(Stream archiveStream) => Init(archiveStream);
        ...
        public SevenZipExtractor(string archiveFullName) => Init(archiveFullName);
        ...
        public SevenZipExtractor(string archiveFullName, string password)
            : base(password) => Init(archiveFullName);
        ...
        public SevenZipExtractor(Stream archiveStream, string password)
            : base(password) => Init(archiveStream);
            ...
            else
            {
                ...
                using ((archiveStream = GetArchiveStream(disposeStream)) as IDisposable)
                {
                    ...
                    if (_filesCount != 0)
                    {
                        ...
                        uint numProps = _archive.GetNumberOfProperties();
                        List<ArchiveProperty> archProps = null;
                        ...
                        try
                        {
                            ...
                            for (uint i = 0; i < _filesCount; i++)
                            {
                                try
                                {
                                    ...
                                    archProps = new List<ArchiveProperty>((int)numProps);
                                    ...
                                    for (uint j = 0; j < numProps; j++)

                                    {
                                        _archive.GetProperty(i, (ItemPropId)j, ref data);

                                        if (PropIdToName.PropIdNames.ContainsKey((ItemPropId)j))
                                        {
                                            archProps.Add(new ArchiveProperty()
                                            {
                                                Name = PropIdToName.PropIdNames[(ItemPropId)j],
                                                Value = data.Object
                                            });
                                        }
                                        else

                                            Debug.WriteLine($"An unknown archive property encountered (code {((int)j).ToString(CultureInfo.InvariantCulture)})");

                                    }

                                    fileInfo._archiveFileInfoProperties = new ReadOnlyCollection<ArchiveProperty>(archProps);
                            ...
                            numProps = _archive.GetNumberOfArchiveProperties();
        ...
        public static byte[] GetLzmaProperties(Stream inStream, out long outSize)
        {
