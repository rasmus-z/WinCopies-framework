...
namespace WinCopies.GUI.Explorer.Data
{
    public class FileSystemInfoAttributesToVisibilityConverter : MultiConverterBase
    {
        ...
        public override object Convert(object[] values, Type targetType, object parameter, CultureInfo culture) => BooleanToVisibilityConverter.Convert(values[0] is IO.ShellObjectInfo && ((ShellObjectInfo)values[0]).ShellObject.IsFileSystemObject
                ? !((FileAttributes)values[1]).HasFlag(FileAttributes.Hidden) && !((FileAttributes)values[1]).HasFlag(FileAttributes.System)
                    ...
                    ? (bool)values[2]
                        ? true
                        : ((IO.ShellObjectInfo)values[0]).FileType == IO.FileTypes.Drive
